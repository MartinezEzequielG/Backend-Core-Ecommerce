...
import { Prisma } from '@prisma/client'; // agrega esta importación
...
export class AdminProductsController {
  constructor(
    ...
    private readonly prisma: PrismaService, // <-- añade prisma
  ) {}
  ...
  // Imágenes
  @Post(':id/images')
  addImage(@Param('id', ParseIntPipe) productId: number, @Body() body: { url: string; position?: number }) {
  ...
  upsertVariant(
    ...
    return this.service.upsertVariant(productId, {
      ...
      optionValueIds: body.optionValueIds ?? [], // ✅ pasar
    });
  }
  ...
  @Post(':id/options')
  addOption(@Param('id', ParseIntPipe) productId: number, @Body() body: { name: string }) {
    return this.service.addOption(productId, body.name);
  }

  @Post('options/:optionId/values')
  addOptionValue(@Param('optionId', ParseIntPipe) optionId: number, @Body() body: { value: string }) {
  ...
  async export(@Res() res: Response) {
    ...
    const header = ['id','name','slug','basePrice','salePrice','active','featured','category','variantsCount','createdAt'];
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(Role.ADMIN, Role.SUPERADMIN)
  async addAudit(@Param('id', ParseIntPipe) id: number, @Body() body: { action: string; details?: any }) {
    return this.prisma.productAudit.create({
      data: { productId: id, action: body.action, details: body.details ?? {}, createdAt: new Date() },
    });
  }
